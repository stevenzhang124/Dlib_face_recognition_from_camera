...
class Face_Recognizer:
    ...
    def draw_note(self, img_rd):
        ...
        cv2.putText(img_rd, "Camera 1", (20, 40), font, 1, (255, 255, 255), 1, cv2.LINE_AA)
        cv2.putText(img_rd, "FPS:   " + str(self.fps.__round__(2)), (20, 100), font, 0.8, (0, 255, 0), 1, cv2.LINE_AA)
        cv2.putText(img_rd, "Persons: " + str(self.faces_cnt), (20, 140), font, 0.8, (0, 255, 0), 1, cv2.LINE_AA)
    ...
    def modify_name_camera_list(self):
        ...
        #self.name_known_list[0] ='Mingjin'.encode('utf-8').decode()
    ...
    def process(self, stream):
        ...
        while self.get_face_database():
            ...
            while stream.isOpened(): 
                    ...
                    if len(faces) != 0:
                        ...
                        for k in range(len(faces)):
                            ...
                            if min(e_distance_list) < 0.4:
                                ...
                                #cv2.rectangle(img_rd, tuple([faces[k].left(), faces[k].top()]), tuple([faces[k].right(), faces[k].bottom()+50]),
                                #              (0, 255, 255), 2)
                            else:
                                #store the unknow person to the database
                                add_new_rows(self.features_camera_list[k])
                                #unknown_list.append(self.features_camera_list[k])
                            ...
                            for kk, d in enumerate(faces):
                                # 绘制矩形框
                                cv2.rectangle(img_rd, tuple([d.left(), d.top()]), tuple([d.right()+50, d.bottom()+200]),
                                              (0, 255, 255), 2)
                            print('\n')
                ...
                if  (end - start) > 5 :  #unknown_list and
                    self.name_known_list = []
                    self.features_known_list = []
                    #add_new_rows(unknown_list)
                    break
            if kk == ord('q'):
                    break
